...
```{r oxygen_profiles_different_geometries, fig.width=6.5, fig.height=5, save_pdf=TRUE}
p1 <- ggplot(data1) +
  ...
  theme_bw() + theme_xangle() + theme_facet(margin.cm = 0.5) + 
  theme_public(font_size = 10) + 
  theme(legend.position = "bottom") 
p1
```

```{r oxygen_profiles_different_geometries_no_legend, fig.width=6.5, fig.height=4, save_pdf=TRUE}
...
```{r oxygen_profiles_variable_diffusivities, fig.width=6.5, fig.height=5, save_pdf=TRUE}
p1 %+% data2  %+%
...
```{r oxygen_profiles_bronchiole_cross_section, fig.width = 6.5, fig.height = 4.5, save_pdf = TRUE}
...
ggplot(data_cart, 
  ...
  scale_fill_gradientn (
    expression("O"[2]~"["*mu*M*"]"), colours = spectral_colors (4), 
    limits=c(0, get_default("o2_max", "µM")), labels = function(x) {
      paste0(x, "                ") # to align legend with growth rates plot
    }) + 
  ...
  theme_bw() + theme_nogrid() + theme_facet(0) + theme_public() + 
  theme_xangle(angle = 90, hjust= 1, vjust = 0.5) + theme_public(font_size = 10) +
  guides(fill = guide_colourbar(barwidth = 1.5, barheight = 10, label.hjust = 0))
```
...
```{r growth_rates_bronchiole_cross_section, fig.width = 6.5, fig.height = 4.5, save_pdf=TRUE}
...
ggplot(data_growth, aes (x, y, fill = colour_scaling(mu.hr))) + 
  ...
  scale_y_continuous(expression("Distance from center of bronchiole ["*mu*m*"]"), labels = function(x) abs(x)) +
  facet_grid(mucus.um~cell.pow, labeller = grid_labeller) + 
  theme_bw() + theme_nogrid() + theme_facet(0) + theme_public() + theme_xangle(angle = 90, hjust= 1, vjust = 0.5) + theme_public(font_size = 10) +
  guides(fill = guide_colourbar(barwidth = 1.5, barheight = 10, label.hjust = 0))
```
